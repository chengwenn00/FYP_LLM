...
if not torch.cuda.is_available():
    rospy.logwarn("CUDA is not available. The model will run on the CPU, which may be slow.")
else:
    rospy.loginfo("CUDA is available. Using GPU for inference.")

# Initialize the Hugging Face Llama model generator pipeline on GPU (device=0)
access_token = "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"  # Replace with your Hugging Face access token
generator = pipeline(
    "text-generation",
    model="meta-llama/Llama-3.1-8B-Instruct",
    use_auth_token=access_token,
    device=0  # Loads the model onto GPU 0; if you have multiple GPUs, adjust accordingly
)
...
# Request navigation instructions from the Llama model (via Hugging Face)
def get_llm_navigation_instructions(start, end, obstacles):
    ...
    try:
        # Request response from the Hugging Face Llama model
        result = generator(prompt, max_length=1024, temperature=0.2, top_p=0.7, do_sample=True)
        # Extract generated text (note: the result may include the prompt text)
        instructions = result[0]['generated_text']
...
# Parse waypoints from the Llama response
...
def generate_smooth_path_with_intermediate_waypoints(waypoints, obstacles, step_size=0.05):
    ...
    for waypoint in waypoints:
        # Calculate the distance between the last point and current waypoint
        distance = sum((waypoint[i] - last_point[i]) ** 2 for i in range(3)) ** 0.5

        # Generate intermediate waypoints for a smooth transition
...
def avoid_obstacles(path, obstacles, step_size):
    ...
    for idx, point in enumerate(path):
        ...
        for obstacle in obstacles:
            ...
            # Define bounding box for the obstacle (2x2x2 units)
...
def create_detour_around_obstacle(start, end, obstacle, clearance_distance=1.1):
    ...
    # Add more waypoints for smoothness in the detour
...
# Execute the trajectory, stopping and holding at the final waypoint
def execute_trajectory(path_instructions):
    ...
    # Define final position coordinates for holding
    final_x, final_y, final_z = path_instructions[-1]

    # Traverse the waypoints, stopping at the final waypoint
    for idx, waypoint in enumerate(path_instructions):
        ...
        # Publish the waypoint multiple times for reliable execution
        for _ in range(10):
            local_pos_pub.publish(pose)
            rate.sleep()

        # If the final waypoint is reached, break the loop and hold position
        if idx == len(path_instructions) - 1:
            rospy.loginfo("Final waypoint reached. Holding position.")
            break

    # Hold final position indefinitely by repeatedly publishing the final position
...
if __name__ == "__main__":
    ...
    end_point = (0.0, -7.0, 0.5)
    
    obstacle_list = [
        (0.0, -1.5, 0.5), (0.0, -5.5, 0.5)
    ]

    llm_waypoints = get_llm_navigation_instructions(start_point, end_point, obstacle_list)
    refined_path = generate_smooth_path_with_intermediate_waypoints(llm_waypoints, obstacle_list)
...
# # Initialize the Hugging Face Llama model generator pipeline
# access_token = "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"  # Replace with your Hugging Face access token
# generator = pipeline("text-generation", model="meta-llama/Llama-3.1-8B-Instruct", use_auth_token=access_token)
