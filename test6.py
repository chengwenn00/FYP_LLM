...
#     # Go through waypoints, stopping at the final waypoint
...
#     # After reaching the final waypoint, hold the specified end_point indefinitely
...
#     end_point = (0.0, -2.0, 1.0)  # Ensure this is the final point to hold at
...
def avoid_obstacles(path, obstacles, step_size):
    ...
    for idx, point in enumerate(path):
        ...
        for obstacle in obstacles:
            obs_x, obs_y, obs_z = obstacle
            # Define bounding box for the obstacle
            if (obs_x - 1 <= point[0] <= obs_x + 1.5 and
                obs_y - 1 <= point[1] <= obs_y + 1.5 and
                obs_z - 1 <= point[2] <= obs_z + 1.5):
                # Obstacle detected within the path
                safe = False
                break
...
if __name__ == "__main__":
    rospy.init_node("drone_navigation_node")
    start_point = (0, 2, 0.5)
    end_point = (-2, -7.0, 0.5)  # Ensure this is the final point to hold at
